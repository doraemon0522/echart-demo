 ...
 * 查询最近N天的时间【始，末】
 * @param {Number} num 天数
 * @param {Boolean} needMoment 是否需要时分秒
 * @param {Number} type timeFormatter函数需要的类型
 */
export function lastNumDays(num = 7, needMoment = true, type = 2) {
  let startSuffix = ''
  let endSuffix = ''
  if (needMoment) {
  ...
  const laskWeekTm = new Date(curDate.setHours(0, 0, 0, 0)).getTime() - 3600 * 1000 * 24 * (num - 1)
  const startTimeStr = `${timeFormatter(laskWeekTm, type)}${startSuffix}`
  const EndTimeStr = `${timeFormatter(curDate.getTime(), type)}${endSuffix}`
